...
describe('UI analyzer', () => {
  ...
  test('returns structured issues', () => {
    const elements: ScannedElement[] = [
      element('button_0', 0, 0, 100, 50, 'Click', 'rgb(120, 120, 120)', 'rgb(130, 130, 130)'),
...
function element(selector: string, x: number, y: number, width: number, height: number, text: string, color: string, backgroundColor: string): ScannedElement {
  return { selector, tagName: 'DIV', text, color, backgroundColor, boundingBox: { x, y, width, height } };
}
