...
async function waitForPort(host: string, port: number, timeoutMs = TARGET_START_TIMEOUT_MS): Promise<void> {
  ...
  throw new Error(`Target app chua mo port ${port} sau ${timeoutMs / 1000}s`);
}
...
async function waitForTargetReady(child: ChildProcess, host: string, port: number): Promise<void> {
    ...
    if (earlyExit) {
      throw new Error(
        `Target app start bi dung som (code ${earlyExit.code ?? 'null'}, signal ${earlyExit.signal ?? 'null'}). ` +
          'Hay kiem tra dependency cua project target, vi du chay install truoc.',
      );
    }
  ...
  throw new Error(`Target app chua mo port ${port} sau ${TARGET_START_TIMEOUT_MS / 1000}s`);
}
...
async function maybeStartTarget(project: ProjectMetadata, projectUrl: string, shouldStart: boolean): Promise<TargetProcess | undefined> {
  ...
  if (await isPortOpen(host, port)) {
    console.log(`[TLX] Target app da chay tai ${projectUrl}`);
    return undefined;
  }
  ...
  if (!command) {
    console.log('[TLX] Khong tim thay command de tu start target app. Hay chay app rieng hoac dung --target-url.');
    return undefined;
  }

  console.log(`[TLX] Target app chua chay. Tu start: ${command}`);
  ...
  console.log(`[TLX] Target app san sang tai ${projectUrl}`);
